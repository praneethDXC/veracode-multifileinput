						...
						<table class="blabbers table">
							<thead>
								<tr>
									...
									<th class="commenterJoinDate"><a href="?sort=<%= Encode.forUri("date_created DESC") %>">Join date</a></th>
									<th class="commenterListeners"><a href="?sort=<%= Encode.forUri("listeners DESC") %>">Listeners</a></th>
									<th></th>
								</tr>
							</thead>
							<tbody>
								...
								<tr>
									...
									<td>
										<form class="form-inline" role="form" method="POST" action="blabbers">
											<input type="hidden" name="blabberUsername" value="<%= Encode.forHtmlAttribute(blabber.getUsername()) %>" />
											<input type="hidden" name="command" value="<%= Encode.forHtmlAttribute(blabber.getNumberListeners() == 1 ? "ignore" : "listen") %>" />
											<input type="submit" class="btn btn-default pull-right" name="button" value="<%= Encode.forHtmlAttribute(blabber.getNumberListeners() == 1 ? "Ignore" : "Listen") %>" />
										</form>
									</td>
								</tr>
								<%
